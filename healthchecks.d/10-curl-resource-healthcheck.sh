...
for i in $(seq 1 "$MAX_RETRIES"); do
    echo "[healthcheck] Checking Resource example.internal status (attempt $i of $MAX_RETRIES)..."
    if curl -s --max-time 10 http://example.internal >/dev/null; then
        echo "[healthcheck] Resource example.internal is reachable via Twingate."
        exit 0
    else
        echo "[healthcheck] Resource example.internal is not reachable. Retrying in $SLEEP_BETWEEN seconds..."
        sleep "$SLEEP_BETWEEN"
    fi
done
